...
class UCBNode:
    ...
    tau_r2: float = -1.0  # -1 means not available
    v_rest_r2: float = -1.0  # -1 means not available
    mutation: str = ""
...
def parse_ucb_scores(filepath: str) -> list[UCBNode]:
    ...
    # All fields after R2 are optional for backward compatibility
    pattern = r'Node (\d+): UCB=([\d.]+), parent=(\d+|root), visits=(\d+), R2=([\d.]+)(?:, Pearson=([\d.]+))?(?:, External_input_R2=([\d.]+))?(?:, Cluster=([\d.]+))?(?:, Time=([\d.]+))?(?:, tau_R2=([\d.]+))?(?:, V_rest_R2=([\d.]+))?(?:, Mutation=([^\n\[]+))?'
    ...
    for match in re.finditer(pattern, content):
        ...
        tau_r2 = float(match.group(10)) if match.group(10) else -1.0
        v_rest_r2 = float(match.group(11)) if match.group(11) else -1.0
        mutation = match.group(12).strip() if match.group(12) else ""
...
def plot_ucb_tree(nodes: list[UCBNode],
    ...
    for node in nodes:
        ...
        # Add tau_R2 and V_rest_R2 if available (flyvis metrics)
        flyvis_parts = []
        if node.tau_r2 >= 0:
            flyvis_parts.append(f"τ={node.tau_r2:.2f}")
        if node.v_rest_r2 >= 0:
            flyvis_parts.append(f"Vr={node.v_rest_r2:.2f}")
        if flyvis_parts:
            label_text += f"\n{', '.join(flyvis_parts)}"
