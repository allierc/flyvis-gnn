...
def data_test_flyvis(config, best_model=None, device=None, log_file=None, test_config=None):
    ...
    logger.info(f'test dataset: {test_ds}, {n_frames} frames, {n_neurons} neurons')
    ...
    if os.path.exists(mask_path):
        ablation_mask = torch.load(mask_path, map_location=device)
        with torch.no_grad():
    ...
    logger.info(f'evaluating on {n_eval_frames} frames ...')
    all_pred = []
    all_true = []
    ...
    logger.info(f'evaluated {len(all_pred)} valid frames')
    ...
    # --- Connectivity and dynamics R² ---
    from flyvis_gnn.metrics import compute_r_squared, get_model_W
    ...
    if 'linear' in model_name:
        ...
        vrest_r2, tau_r2, connectivity_r2 = compute_dynamics_r2_linear(model, config, device, n_neurons)

    elif 'MLP' not in model_name:
        # GNN: corrected-weight R2 and slope-based dynamics R2
        from flyvis_gnn.metrics import compute_all_corrected_weights, compute_dynamics_r2
        gt_weights = torch.load(graphs_data_path(config.dataset, 'weights.pt'), map_location=device)
        gt_w_np = to_numpy(gt_weights)
        corrected_W, _, _, _ = compute_all_corrected_weights(model, config, edges, x_ts_eval, device)
        try:
            corrected_r2, _ = compute_r_squared(gt_w_np, to_numpy(corrected_W.squeeze()))
        except Exception:
            corrected_r2 = 0.0
        vrest_r2, tau_r2 = compute_dynamics_r2(model, x_ts_eval, config, device, n_neurons)
        connectivity_r2 = corrected_r2  # report corrected for GNN

    logger.info(f'connectivity R²: {connectivity_r2:.4f}')
    logger.info(f'tau R²: {tau_r2:.4f}')
    logger.info(f'V_rest R²: {vrest_r2:.4f}')

    # Cluster accuracy (embedding-based, GNN only)
    cluster_accuracy = None
    if 'linear' not in model_name and 'MLP' not in model_name:
        from flyvis_gnn.sparsify import clustering_evaluation
        eps = getattr(tc, 'cluster_distance_threshold', 0.1)
        results = clustering_evaluation(to_numpy(model.a[:n_neurons].detach()), type_list, eps=eps)
        cluster_accuracy = results['accuracy']
        logger.info(f'cluster accuracy: {cluster_accuracy:.4f} ({results["n_clusters_found"]} clusters)')
    ...
    with open(results_path, 'w') as f:
        ...
        f.write(f'connectivity_R2: {connectivity_r2:.4f}\n')
        f.write(f'tau_R2: {tau_r2:.4f}\n')
        f.write(f'V_rest_R2: {vrest_r2:.4f}\n')
        if cluster_accuracy is not None:
            f.write(f'cluster_accuracy: {cluster_accuracy:.4f}\n')
    ...
    if log_file:
        ...
        if cluster_accuracy is not None:
            log_file.write(f'cluster_accuracy: {cluster_accuracy:.4f}\n')
    ...
    logger.info(f'rollout metrics saved to {rollout_log_path}')
    ...
    logger.info(f'rollout plots saved to {results_dir}/')
