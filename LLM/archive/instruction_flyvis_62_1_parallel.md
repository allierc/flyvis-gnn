# Parallel Mode Addendum — FlyVis

This addendum applies when running in **parallel mode** (GNN_LLM_parallel_flyvis.py). Follow all rules from the base instruction file, with these modifications.
...
- Suggested spread: vary `learning_rate_W_start` across the range (e.g. 5E-4, 1E-3, 2E-3, 5E-3)
...
- Reduce complexity in the next mutation (smaller hidden_dim, lower data_augmentation_loop, smaller batch_size)
