...
def _create_learned_model(config, device):
    """Create a fresh learned model (FlyVisGNN) for loading trained weights into."""
...
def create_signal_excitation_subplot(fig, ax, model, config, n_frames, mc, device,
    ...
    gt_max_amplitude = getattr(sim, 'oscillation_max_amplitude', 1.0)
    gt_excitation = gt_max_amplitude * np.cos((2 * np.pi) * sim.oscillation_frequency * frame_)
...
def plot_signal(config, epoch_list, log_dir, logger, cc, style, extended, device, apply_weight_correction=False, log_file=None):
    ...
    if (sim.external_input_type != '') & (sim.external_input_type!='none'):
    ...
    if not os.path.exists(svd_plot_path):
        ...
        try:
            analyze_data_svd(x_ts, log_dir, config=config, logger=logger, style=svd_style)
    ...
    type_list = x_ts.neuron_type.to(device).unsqueeze(-1)  # (N, 1) for compat
    ...
    if has_external_input:
        if ('short_term_plasticity' in sim.external_input_type) | ('modulation_permutation' in sim.external_input_type):
    ...
    if epoch_list[0] == 'all':
        ...
        with torch.no_grad():
            for file_id_ in trange(0,len(file_id_list), ncols=90):
                ...
                if has_external_input:
                    ...
                    if 'short_term_plasticity' in sim.external_input_type:
                        fig, ax = fig_style.figure()
        ...
        true_model, bc_pos, bc_dpos = _create_true_model(config=config, W=adjacency, device=device)
        ...
        for epoch in epoch_list:
            ...
            # create 2x3 figure
            fig, axes = fig_style.figure(nrows=2, ncols=3)
            ...
            fig_style.xlabel(axes[0, 0], 'real')
            fig_style.ylabel(axes[0, 0], 'imag')
            axes[0, 0].legend(fontsize=fig_style.tick_font_size)
            axes[0, 0].set_title('eigenvalues in complex plane', fontsize=fig_style.font_size)

            # (0,1) eigenvalue magnitude comparison (sorted)
            axes[0, 1].scatter(np.abs(eig_true_sorted), np.abs(eig_learned_sorted), s=20, c='black', alpha=0.7)
            max_val = max(np.abs(eig_true_sorted).max(), np.abs(eig_learned_sorted).max())
            axes[0, 1].plot([0, max_val], [0, max_val], 'g--', linewidth=fig_style.line_width)
            fig_style.xlabel(axes[0, 1], 'true |eigenvalue|')
            fig_style.ylabel(axes[0, 1], 'learned |eigenvalue|')
            axes[0, 1].set_title('eigenvalue magnitude comparison', fontsize=fig_style.font_size)

            # (0,2) eigenvalue spectrum (log scale)
            axes[0, 2].plot(np.abs(eig_true_sorted), color='green', linewidth=fig_style.line_width, label='true')
            axes[0, 2].plot(np.abs(eig_learned_sorted), color='black', linewidth=fig_style.line_width, label='learned')
            fig_style.xlabel(axes[0, 2], 'index')
            fig_style.ylabel(axes[0, 2], '|eigenvalue|')
            axes[0, 2].set_yscale('log')
            axes[0, 2].legend(fontsize=fig_style.tick_font_size)
            axes[0, 2].set_title('eigenvalue spectrum (log scale)', fontsize=fig_style.font_size)

            # Row 2: Eigenvectors
            # (1,0) right eigenvector alignment matrix
            im = axes[1, 0].imshow(alignment_R, cmap='hot', vmin=0, vmax=1)
            fig_style.xlabel(axes[1, 0], 'learned eigenvector index')
            fig_style.ylabel(axes[1, 0], 'true eigenvector index')
            axes[1, 0].set_title('right eigenvector alignment', fontsize=fig_style.font_size)
            plt.colorbar(im, ax=axes[1, 0], fraction=0.046)

            # (1,1) left eigenvector alignment matrix
            im_L = axes[1, 1].imshow(alignment_L, cmap='hot', vmin=0, vmax=1)
            fig_style.xlabel(axes[1, 1], 'learned eigenvector index')
            fig_style.ylabel(axes[1, 1], 'true eigenvector index')
            axes[1, 1].set_title('left eigenvector alignment', fontsize=fig_style.font_size)
            ...
            axes[1, 2].axhline(y=1/np.sqrt(n_plot), color='gray', linestyle='--', linewidth=fig_style.line_width, label=f'random ({1/np.sqrt(n_plot):.2f})')
            fig_style.xlabel(axes[1, 2], 'eigenvector index (sorted by |eigenvalue|)')
            fig_style.ylabel(axes[1, 2], 'best alignment score')
            axes[1, 2].set_title('best alignment per eigenvector', fontsize=fig_style.font_size)
            axes[1, 2].set_ylim([0, 1.05])
            axes[1, 2].legend(fontsize=fig_style.tick_font_size)

            plt.tight_layout()
            fig_style.savefig(fig, f"{log_dir}/results/eigen_comparison.png")
            plt.close()
            ...
            if 'low_rank' in sim.connectivity_type:
                ...
                if os.path.exists(U_true_path) and os.path.exists(V_true_path):
                    ...
                    U_est, V_est, W_recon = recover_UV_from_missing_diagonal(learned_W, sim.connectivity_rank)
